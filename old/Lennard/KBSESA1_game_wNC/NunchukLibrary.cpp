...
{
if((analogX - beginPossX) > 5)		status = n_RIGHT;//X AXIS RIGHT
else if((beginPossX - analogX) > 5) status = n_LEFT;//X AXIS LEFT
else								status = n_IDLE;//X AXIS NO MOVEMENT
if((analogY - beginPossY) > 5)		status = n_UP;//Y AXIS UP
else if((beginPossY - analogY) > 5) status = n_DOWN;//Y AXIS DOWN
}
*/
